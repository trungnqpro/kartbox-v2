              ...
              >
                <slot :name="item.slot || 'item'" :item="item">
                  <UIcon
                    ...
                    :class="[
                      ui.item.icon.base,
                      active ? ui.item.icon.active : ui.item.icon.inactive,
                      item.iconClass,
                    ]"
                  />
                  <UAvatar
                    v-else-if="item.avatar"
                    v-bind="{ size: ui.item.avatar.size, ...item.avatar }"
                    :class="ui.item.avatar.base"
                  />
                  ...
                  >
                    <UKbd v-for="shortcut of item.shortcuts" :key="shortcut">{{
                      shortcut
                    }}</UKbd>
                  </span>
                </slot>
...
import UKbd from '../Kbd/index.vue'
import ULink from '../Link/index.vue'
  ...
  setup(props) {
    ...
    const containerStyle = computed(() => {
      const offsetDistance =
        (props.popper as PopperOptions)?.offsetDistance ||
        (ui.value.popper as PopperOptions)?.offsetDistance ||
        8

      return props.mode === 'hover'
        ? {
            paddingTop: `${offsetDistance}px`,
            paddingBottom: `${offsetDistance}px`,
          }
        : {}
    })
